...
function fixPostalCode(postalCode) {
  ...
  if (postalCode.length === 6) {
    let leftHalf = postalCode.slice(0, 3);
    let rightHalf = postalCode.slice(3, 6);
    postalCode = leftHalf + ' ' + rightHalf;
  }
  ...
  if (
    postalCode.search(/[DFIOQWUZ]/) == 0 ||
    postalCode.search(/[DFIOQU]/) == 2 ||
    postalCode[3] != ' ' ||
    postalCode.search(/[DFIOQU]/) == 5 ||
    isNaN(postalCode[1]) ||
    isNaN(postalCode[4]) ||
    isNaN(postalCode[6])
  ) {
...
function toProvince(postalCode, useShortForm) {
  ...
  if (postalCode[0].match(/[KLMNP]/)) {
    return useShortForm ? 'ON' : 'Ontario';
  } else if (postalCode[0].match(/[GHJ]/)) {
    return useShortForm ? 'QC' : 'Quebec';
  } else if (postalCode[0].match('B')) {
    return useShortForm ? 'NS' : 'Nova Scotia';
  } else if (postalCode[0].match('E')) {
    return useShortForm ? 'NB' : 'New Brunswick';
  } else if (postalCode[0].match('R')) {
    return useShortForm ? 'MB' : 'Manitoba';
  } else if (postalCode[0].match('V')) {
    return useShortForm ? 'BC' : 'British Columbia';
  } else if (postalCode[0].match('C')) {
    return useShortForm ? 'PE' : 'Prince Edward Island';
  } else if (postalCode[0].match('S')) {
    return useShortForm ? 'SK' : 'Saskatchewan';
  } else if (postalCode[0].match('T')) {
    return useShortForm ? 'AB' : 'Alberta';
  } else if (postalCode[0].match('A')) {
    return useShortForm ? 'NL' : 'Newfoundland and Labrador';
  } else if (postalCode[0].match('X')) {
    return useShortForm ? 'NT' : 'Northwest Territories and Nunavut';
  } else if (postalCode[0].match('Y')) {
    return useShortForm ? 'YT' : 'Yukon';
  }
  return null;
}
...
function normalizeCoord(value) {
  ...
  console.log(value);
  if (value.startsWith('[')) {
    // this will remove the square bracket
...
function formatCoords(...values) {
  ...
  for (let i = 0; i < values.length; i++) {
    include = 1;
    try {
      normalizeCoord(values[i]);
    } catch (E) {
      include = 0;
    }
    if (include == 1) {
      arr += normalizeCoord(values[i]);
...
function countForProvince(provinceCode, ...postalCodes) {
  ...
  for (let i = 0; i < postalCodes.length; i++) {
    ...
    if (typeof postalCodes[i] !== 'string') {
      throw postalCodes[i];
    }
...
function generateLicenseLink(licenseCode, targetBlank) {
  if (licenseCode) {
    licenseCode = licenseCode.slice(3, licenseCode.length).toLowerCase();
  }
  let returnValue = '<a href="https://creativecommons.org/licenses/' + licenseCode + '/4.0/"';
  //returnValue += '/4.0/"';
  if (targetBlank) {
  ...
  switch (licenseCode) {
    case 'by':
      returnValue += '>Creative Commons Attribution License';
      break;
    case 'by-nc':
      returnValue += '>Creative Commons Attribution-NonCommercial License';
      break;
    case 'by-sa':
      returnValue += '>Creative Commons Attribution-ShareAlike License';
      break;
    case 'by-nd':
      returnValue += '>Creative Commons Attribution-NoDerivs License';
      break;
    case 'by-nc-sa':
      returnValue += '>Creative Commons Attribution-NonCommercial-ShareAlike License';
      break;
    case 'by-nc-nd':
      returnValue += '>Creative Commons Attribution-NonCommercial-NoDerivs License';
      break;
    default:
...
function pureBool(value) {
  if (typeof value === 'string') {
    if (value.match(/^[yotv]/i)) {
      return true;
    } else if (value.match(/^[NF]/i)) {
      return false;
    } else {
      throw 'invalid value';
    }
...
function buildUrl(query, order, count, license) {
  ...
  let returnValue = 'https://api.inaturalist.org/v2/observations';
  if (query) {
    returnValue += `?query=${encodeURIComponent(query)}`;
  }
  if (count < 1 || count >= 31) {
    throw 'error';
  }
  ...
  if (order.match(/asc/) || order.match(/desc/)) {
    if (order == 'asc') {
      returnValue += `&order=ascending`;
    } else if (order == 'desc') {
      returnValue += `&order=descending`;
    } else {
