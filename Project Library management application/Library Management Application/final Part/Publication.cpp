...
namespace sdds
{
	...
	void Publication::set(int member_id)
	{
		...
		else if (member_id > 9999 && member_id < 100000) {
	...
	std::ostream& Publication::write(std::ostream& os) const
	{
		if (conIO(os))
		{
			...
			os.setf(ios::left);
			U.writeWithLen(os, m_title, SDDS_TITLE_WIDTH, FILL_CHAR);
			os<< " | ";
			if (m_membership)
			{
				os << m_membership << " | ";
	...
	std::istream& Publication::read(std::istream& istr)
	{
		...
		if (conIO(istr))
		{
			...
			if (U.strlen(shelfId) != SDDS_SHELF_ID_LEN)
			{
				istr.setstate(ios_base::failbit);
			}
			cout << "Title: ";
			istr.getline(title, MAX_TITLE+1, '\n');
			cout << "Date: ";
			date.read(istr);
		}
		else {
