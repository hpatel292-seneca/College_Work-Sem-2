...
namespace sdds {
   const int MAX_CHAPTER_TITLE = 20;
    ...
    public:
        ...
        std::ostream& display(std::ostream&)const;// having doubt about the ostream object.
        operator int()const;
        operator double()const;
        operator const char* ()const;
        const Chapter& operator=(const Chapter& right);// When would i set an return type to const??
        Chapter& operator++();
        Chapter& operator--();
    };
